...
int main(int argc, char** argv)
{
	...
	const int delay = 100 * 1000; /* micro seconds */
	...
	printf("tuning reg %u(%#02x) from %u(%#02x) to %u(%#02x)\n", reg, reg, oldval, oldval, val, val);
	...
	oldval += inc;
	while (oldval != (val + inc)) {
		smbioctl_writeB(slave, reg, oldval);
		oldval += inc;
		usleep(delay);
	}

	val = smbioctl_readB(slave, reg);
	printf("reg %u(%#02x) = %u(%#02x)\n", reg, reg, val, val);
